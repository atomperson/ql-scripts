...
let userCookie = 'PPU=UID=88693105101867&UN=eoorptgar&TT=82fa6f25b060efd8b307290c7c0891e9&PBODY=GSSc43-RAknPkFjgR6s-eHIFj79iCrVxBVGuMEQM9SjDtGFkTH0XyuQxswigbEte51mEDntdSRJ8TqdpMXwlPNyh_QFCBCLYP7tvJT4r9v0flm7n_SOPnIDyIGfIFBiYFGvCPqqmbuez9Ffk8aoIDfv4oddqQjr6MMqFTsmVh9A&VER=1&CUID=8sTBgQ91PmySpvjJLQX5XQ#1'
...
//($.isNode() ? process.env[ckkey] : $.getdata(ckkey)) || '';

let userUA ='Dalvik/2.1.0 (Linux; U; Android 12; M2012K11AC Build/SKQ1.220303.001)'
//($.isNode() ? process.env.wbtcUA : $.getdata('wbtcUA')) || 'Mozilla/5.0 (iPhone; CPU iPhone OS 15_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 WUBA/10.26.5';
let userList = []

let doHomeContinueTaskNum = 2; // 首页签到再赚矿石任务最大次数
...
let shields = ['13-2438', '4-2653', '4-2747', '4-5010', '4-5027', '9-5030', '9-5031', '20-2773', '20-5028', '20-5029'] // 屏蔽任务
...
class UserInfo {
    ...
    async checkLogin2() {
        ...
        let body = ``
        let urlObject = populateUrlObject(url, this.cookie, body)
        let {data} = await axios.get(url, urlObject )
        if (!data) return
        // console.log(JSON.stringify(result))
        if (data.code == 0) {
            this.login = true
    ...
    async checkLogin3() {
        ...
        let result = httpResult;
        //let {data} = await axios.post(url, urlObject )
        if (!result) return
        // console.log(JSON.stringify(result))
        if (result.code == 0) {
    ...
    async cashSigninlist() {
        ...
        if (result.code == 0) {
            ...
            console.log(`账号[${this.index}-${this.nickName}]今日现金签到页: ${cashStr}`)
        } else {
            console.log(`账号[${this.index}-${this.nickName}]查询现金签到失败: ${result.message}`)
        }
    }
    ...
    async cashSignin() {
        ...
        //console.log(result)
        if (result.code == 0) {
            console.log(`账号[${this.index}-${this.nickName}]现金签到获得${result.data.amount}元`)
        } else {
            console.log(`账号[${this.index}-${this.nickName}]查询现金签到失败: ${result.message}`)
        }
    }
    ...
    async newbieMaininfo() {
        ...
        if (result.code == 0) {
            ...
            if (this.newbie.isWithdraw == false) {
                let sortList = result.result.withdrawInfo.sort(function (a, b) {
                    return b.cardAmount - a.cardAmount
                })
    ...
    async getTaskList(sceneId) {
        ...
        if (result.code == 0) {
            ...
            let max = Math.ceil(taskList.length / 3)
    ...
    async getWalletInfo() {
        ...
        // console.log(result)
        if (result.code == 0) {
            console.log(`账号[${this.index}-${this.nickName}]钱包余额: ${result.result.dataList[0].balance}, ${result.result.dataList[0].subTitle}元`)
        } else {
    ...
    async oreMainpage(dotask = true) {
        ...
        if (result.code == 0) {
            ...
            if (dotask) {
                ...
                console.log(`账号[${this.index}-${this.nickName}]今日神奇矿${dailyStr}，${signStr}，${rentStr}，参加三个小游戏任务${gameStr}`)
                if (this.ore.sign == 0) {
                    await $.wait(500)
                    // 神奇矿签到
                    await this.oreSign()
                ...
                console.log(`账号[${this.index}-${this.nickName}]神奇矿余额${this.ore.ore} ≈ ${this.ore.money}元`)
            }
        } else {
    ...
    async doWakeupEmploy() {
        ...
        if (type == 1 || type == 2) {
            ...
            if (res) {
                ...
                await $.wait(200)
                await this.getMineMaininfo();
                console.log(`账号[${this.index}-${this.nickName}]唤醒金币矿工成功, 挖矿10分钟`)
            }
        }
    }
            ...
            if (result.result.reward.length) {
                for (let i = 0; i < result.result.reward.length; i++) {
                    ...
                    if (reward.type === 1) {
                        ...
                        console.log(`邀请新人活动 ：账号[${this.index}]红包${i + 1}:${reward.actualNumber}元，${reward.currentNumber}/${reward.total} ${doneStrMap[reward.awardState]}`);
                    } else {
                        console.log(`账号[${this.index}]领取到矿石${result.message}`);
                        // type 1-邀请红包 2-矿石（需领取）3-转盘+1次 未知 4-谢谢参与
                        await $.wait(500);
                        this.receiveOre(reward.actualNumber);
    ...
    async offlineRevenue() {
        ...
        console.log(JSON.stringify(result))
        if (result.code == 0) {
            console.log(`账号[${this.index}-${this.nickName}]神奇矿石离线矿石收取成功, 获得${mineCarOre}mg矿石`)
        } else {
    ...
    async upgrade() {
        ...
        // console.log(JSON.stringify(result))
        if (result.code == 0) {
            console.log(`账号[${this.index}-${this.nickName}]神奇矿石升级装备成功, 升级为${equipmentLevel + 1}级装备`)
        } else {
...
!(async () => {
    if (typeof $request !== "undefined") {
        ...
        for (let user of userList) {
            ...
            console.log(`账号[${user.index}-${user.nickName}]已经开采${cumulativeOre}mg，再开采${dix}mg可领取${user.mineMaininfo.superInfo.thousandOreValue}元`)
            await $.wait(500);
        ...
        // console.log('\n======== 金币任务 ========')
        // // 未绑定微信账号不能参加
        // if(curHour>=TASK_TIME[0] && curHour<TASK_TIME[1]) {zhej
        //     console.log('\n查询任务...')
        ...
        //         const candoList = userList.filter(x => i<x.task.length);
        //         for(let [k, user] of candoList.entries()) {
        //             if (['1','2'].includes(user.runTask)) {
        ...
        //                     // 等到25s
        //                     if (WAIT_TASK25.includes(taskIdStr)) {
        //                         await $.wait(25000);
        //                     }
        ...
        //                     if (WAIT_TASK10.includes(item.sceneId) && k!=candoList.length-1) {
        //                         await $.wait(10500);
        //                     }
        //                     // 以下sceneId分类下的任务运行时间间隔需大于20s,避免执行任务异常
        //                     if (WAIT_TASK20.includes(item.sceneId) && k!=candoList.length-1) {
        //                         await $.wait(20000);
        //                     }
        //                 } else {
...
async function httpRequest(method, url) {
    ...
    let {data} =await axios.post(url.url, url.body,{headers:url.headers} );
    httpResult = data
