...
微信打开：http://www.aircvr.cn?masterid=oFFbU5l0IGFqsD3uBPMHrWsLHEWc

变量 export cdbcookie="unionid=123&token=456@unionid=123&token=456"
...
/*
    美团充电宝
 [Script]
    cron "0 1 * * *" script-path=kbg_58sjb.js, tag=58同城, enabled=true
*/
const $ = new Env("美团充电宝");
    ...
    async getuseinfo() {
        try {
		  ...
		  if (alipay)var newalipay = alipay.substr(0,3) + "****" + alipay.substr(7)
                console.log(`\n用户 `+result.result.nickname+`  余额 ${this.money}元 邀请数 `+result.result.lvone)
