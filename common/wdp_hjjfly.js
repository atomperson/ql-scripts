/*
@wdp  7.27 航嘉积分乐园 积分兑换实物
一天1-2次吧。
[task_local]
#航嘉积分乐园
45 7,12,20 * * * wdp_casl.js, tag=航嘉积分乐园, enabled=true
定时：一天一两次
*/
...
//let hjjflyToken='bearer 6e349086-8b5a-43f1-9048-594f8a7b3e03\n' +
    'bearer 54257b76-46c3-4901-acef-d237e1b9a0be\n' +
    'bearer 189a9e9a-3011-4648-a992-91d59ef2c161';
...
!(async () => {
    if (typeof $request !== "undefined") {
        ...
        console.log(`\n=================== 共找到 ${hjjflyTokenArr.length} 个账号 ===================`)
        //随机诗词
        await poem();

        for (let index = 0; index < hjjflyTokenArr.length; index++) {
...
async function account_detail(index) {
    let url = 'https://wx.huntkey.cn/aliyun/v1/customer/integral';
    let body = ''
    ...
    console.log('用户信息查询成功')
     var score = result.content.total;
    //console.log(JSON.stringify(userinfo));
    addNotifyStr(`账户 ${index}: 账户得分为${score}\n`, true)
}
//签到
async function sign() {
    let url = 'https://wx.huntkey.cn/aliyun/v1/integral/gain/signIn';
    let body = ''
    let urlObject = populateUrlObject(url, '', body)
...
async function Envs() {
    if (hjjflyToken) {
        if (hjjflyToken.indexOf("@") != -1) {
            hjjflyToken.split("@").forEach((item) => {
                hjjflyTokenArr.push(item);
            });
        } else if (hjjflyToken.indexOf("\n") != -1) {
            hjjflyToken.split("\n").forEach((item) => {
                hjjflyTokenArr.push(item);
            });
        } else {
            hjjflyTokenArr.push(hjjflyToken);
        }
    } else {
        log(`\n提示：未填写提现变量，不会执行自动提现`)
    }
